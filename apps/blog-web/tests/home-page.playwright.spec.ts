...
test.describe('[UI] 홈 화면', () => {
  test('[UI] 핵심 콘텐츠를 노출한다', async ({ page }) => {
    ...
    await expect(page.getByText('전체 포스트')).toBeVisible()
    await expect(page.getByText('2개')).toBeVisible()
    await expect(page.getByText('카테고리 & 태그')).toBeVisible()
    await expect(page.getByText('5 카테고리')).toBeVisible()
    await expect(page.getByText('4개의 태그 수록')).toBeVisible()
    await expect(page.getByText('마지막 업데이트')).toBeVisible()
    await expect(page.getByText('2025년 10월 1일').first()).toBeVisible()

    await expect(page.getByRole('heading', { name: '가장 주목받는 포스트' })).toBeVisible()
    await expect(page.getByRole('link', { name: '모든 포스트 보기' })).toBeVisible()
    await expect(page.getByRole('link', { name: 'NextAuth' })).toBeVisible()
    await expect(page.getByText('NextAuth에 대한 설명 입니다.')).toBeVisible()
    await expect(page.getByText('12 views')).toBeVisible()

    await expect(page.getByRole('heading', { name: '지금 가장 많이 읽는 주제' })).toBeVisible()
    await expect(page.getByRole('link', { name: '검색으로 더 찾아보기' })).toBeVisible()
    await expect(page.getByRole('heading', { name: '인기 포스트 Top 1' })).toBeVisible()
    await expect(page.getByText('최종 성공 포스트 - MCP 수정 확인').first()).toBeVisible()
    await expect(page.getByText('21회 조회')).toBeVisible()

    await expect(page.getByRole('heading', { name: '카테고리' })).toBeVisible()
    await expect(page.getByRole('link', { name: '개발' }).first()).toBeVisible()
    await expect(page.getByRole('link', { name: '으아아이' }).first()).toBeVisible()
    await expect(page.getByRole('link', { name: '프론트엔드' }).first()).toBeVisible()

    await expect(page.getByRole('heading', { name: '인기 태그' })).toBeVisible()
